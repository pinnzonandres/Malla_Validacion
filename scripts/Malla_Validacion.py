...
class validacion:
    ...
    def get_malla(self):
        if self.bool_malla:
             ruta = '..\\data\\validation_json\\{}.json'.format(self.nombre_malla)
             with open(ruta, encoding='utf-8') as file:
                 self.malla = json.load(file)
        else:
            ruta =  '..\\data\\validation_excel\\{}.xlsx'.format(self.nombre_malla)
            ruta_export = '..\\data\\validation_json\\{}.json'.format(self.nombre_malla)
            try:
                cond = pd.read_excel(ruta, sheet_name='Validaciones')
                val = pd.read_excel(ruta, sheet_name='Valores')
                val = val.dropna(subset=['Valores','Tipo Validacion'])
            except Exception as e:
